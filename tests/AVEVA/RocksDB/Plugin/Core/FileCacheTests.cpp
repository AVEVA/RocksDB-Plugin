...
public:
    ...
    {
        EXPECT_CALL(*m_filesystem, DeleteFile(_))
            .WillRepeatedly([this](const std::filesystem::path& path)
                {
...
TEST_F(FileCacheTests, ReadFileFromCache)
{
        ...
        .WillRepeatedly(Invoke([&fileData](const std::filesystem::path&)
            {
                ...
                EXPECT_CALL(*file, Read(_, _, _))
                    .WillRepeatedly([&fileData](char* buffer, uint64_t offset, uint64_t length) -> uint64_t
                        {
                            std::copy(fileData.begin() + offset, fileData.begin() + offset + length, buffer);
                            return length - offset;
                        });
                return file;
            }));
    ...
    const auto bytesRead = m_cache.ReadFile("1.sst", 0, fileData.size(), buffer.data());
