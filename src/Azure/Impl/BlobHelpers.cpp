...
int64_t BlobHelpers::GetFileSize(const ::Azure::Storage::Blobs::PageBlobClient& client) {
    ...
    return metaIter != props.Value.Metadata.end() ? static_cast<int64_t>(std::stoll(metaIter->second)) : 0;
}
...
BlobHelpers::CreateServiceClient(const Models::ServicePrincipalStorageInfo& servicePrincipal) {
    auto clientSecretOptions = CreateClientSecretCredentialOptions();
    auto cred = std::make_shared<::Azure::Identity::ClientSecretCredential>(
        servicePrincipal.GetTenantId(), servicePrincipal.GetServicePrincipalId(),
        servicePrincipal.GetServicePrincipalSecret(), clientSecretOptions);

    auto blobOptions = CreateBlobClientOptions();
    return ::Azure::Storage::Blobs::BlobServiceClient{servicePrincipal.GetStorageAccountUrl(), std::move(cred),
                                                      blobOptions};
}

::Azure::Storage::Blobs::BlobServiceClient
BlobHelpers::CreateServiceClient(const Models::ChainedCredentialInfo& chainedCredential) {
    ...
    credSources.push_back(std::make_shared<::Azure::Identity::ClientSecretCredential>(
        chainedCredential.GetTenantId(), chainedCredential.GetServicePrincipalId(),
        chainedCredential.GetServicePrincipalSecret(), clientSecretOptions));
    ...
    auto blobOptions = CreateBlobClientOptions();
    return ::Azure::Storage::Blobs::BlobServiceClient{chainedCredential.GetStorageAccountUrl(), std::move(cred),
                                                      blobOptions};
}

::Azure::Storage::Blobs::BlobContainerClient
BlobHelpers::GetContainerClient(::Azure::Storage::Blobs::BlobServiceClient& blobServiceClient,
