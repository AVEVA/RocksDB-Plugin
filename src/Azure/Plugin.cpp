...
Plugin::Register(rocksdb::ConfigOptions& configOptions, rocksdb::Env** env, std::shared_ptr<rocksdb::Env>* guard,
    ...
    if (rocksdb::ObjectLibrary::Default()->FindFactory<rocksdb::FileSystem>(pluginName) == nullptr) {
        rocksdb::ObjectLibrary::Default()->AddFactory<rocksdb::FileSystem>(
            pluginName,
            [=, primary = std::move(primary), backup = std::move(backup), logger = std::move(logger)](
                const std::string& /* uri */, std::unique_ptr<rocksdb::FileSystem>* f, std::string* /* errmsg */) {
                auto impl = std::make_unique<Impl::BlobFilesystemImpl>(
                    primary, backup, dataFileInitialSize, dataFileBufferSize, logger, cachePath, maxCacheSize);

                *f = std::unique_ptr<rocksdb::FileSystem>(
    ...
    if (rocksdb::ObjectLibrary::Default()->FindFactory<rocksdb::FileSystem>(pluginName) == nullptr) {
        rocksdb::ObjectLibrary::Default()->AddFactory<rocksdb::FileSystem>(
            ...
            [=, primary = std::move(primary), backup = std::move(backup), logger = std::move(logger)](
                ...
                *f = std::unique_ptr<rocksdb::FileSystem>(
                    new BlobFilesystem(rocksdb::FileSystem::Default(), std::move(impl), std::move(logger)));
                return f->get();
            });
    }
