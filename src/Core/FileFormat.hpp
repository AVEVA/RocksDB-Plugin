...
namespace AVEVA::RocksDB::Plugin::Core
{
    ...
    struct FileFormat
    {
        ...
        /// packs to exactly 18 bytes with no compiler padding.
        /// </summary>
        struct Header
        {
    ...
    static_assert(sizeof(FileFormat::Header) == sizeof(uint64_t) + 2 * sizeof(uint8_t) + sizeof(uint64_t),
        "FileFormat::Header must be exactly 18 bytes with no padding");
}
