...
class LruFileIndex {
  public:
    ...
    class ScopedPin {
      public:
        ...
        ScopedPin(ScopedPin&& o) noexcept
            : m_owner(std::exchange(o.m_owner, nullptr)), m_filename(std::move(o.m_filename)) {}
      ...
      private:
        ...
        ScopedPin(LruFileIndex* owner, std::string_view filename) : m_owner(owner) {
    ...
    /// </summary>
    [[nodiscard]] std::optional<EvictList> ReserveCapacity(std::string_view filename, size_t storedSize,
                                                           bool force_insert);
    ...
    /// </returns>
    [[nodiscard]] std::pair<std::string, std::string> Remove(std::string_view filename) noexcept;
