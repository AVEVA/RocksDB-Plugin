...
class Filesystem {
  public:
    ...
    /// read error occurs.
    /// </summary>
    virtual std::optional<std::string> ReadFileContents(const std::filesystem::path& path) noexcept = 0;
    ...
    /// </summary>
    virtual bool WriteFileAtomic(const std::filesystem::path& finalPath, const char* data, size_t size) noexcept = 0;

    /// <summary>
    /// Renames <paramref name="from"/> to <paramref name="to"/>.
    /// Returns <c>true</c> on success.
    /// </summary>
    virtual bool RenameFile(const std::filesystem::path& from, const std::filesystem::path& to) noexcept = 0;
};
} // namespace AVEVA::RocksDB::Plugin::Core
